...
def test_environment(project_root: Path, state_root: Path, cache_root: Path | None = None) -> dict[str, str]:
    env = os.environ.copy()
    package_roots = []
    fallback = project_root / "src"
    repository_root = Path(__file__).resolve().parents[1] / "src"
    if fallback.exists():
        package_roots.append(str(fallback))
    if str(repository_root) not in package_roots:
        package_roots.append(str(repository_root))
